...
async function main() {
  ...
  try {
    ...
    performances.forEach((perf, index) => {
      ...
      console.log(
        `   Overall Score: ${perf.overallScore.toFixed(3)} (${perf.successfulGenerations}/${
          perf.totalTests
        } successful)`
      );
      console.log(
        `   Answer Correctness: ${perf.criteria.answerCorrectness.toFixed(
          3
        )} | Exercise Design: ${perf.criteria.exerciseDesign.toFixed(
          3
        )} | Explanations: ${perf.criteria.explanationQuality.toFixed(3)}`
      );
      console.log(
        `   Speed & Reliability: ${perf.criteria.speedReliability.toFixed(3)}${
          perf.criteria.costEfficiency ? ` | Cost Efficiency: ${perf.criteria.costEfficiency.toFixed(3)}` : ""
        }`
      );
    ...
    exerciseTypes.forEach((exerciseType) => {
      ...
      performances.forEach((perf) => {
        const typeResults = perf.results.filter(
          (r) => ALL_TEST_CASES.find((tc) => tc.id === r.testCaseId)?.request.exerciseType === exerciseType
        );

        if (typeResults.length > 0) {
          const successfulCount = typeResults.filter((r) => r.exerciseGenerated).length;
          const avgScore =
            typeResults.length > 0 ? typeResults.reduce((sum, r) => sum + r.overallScore, 0) / typeResults.length : 0;
    ...
    performances.forEach((perf) => {
      ...
      const lowestScoring = successfulResults.sort((a, b) => a.overallScore - b.overallScore).slice(0, 3);
      ...
      if (lowestScoring.length > 0) {
        ...
        lowestScoring.forEach((result) => {
          const testCase = ALL_TEST_CASES.find((tc) => tc.id === result.testCaseId);
          console.log(
            `  • ${result.testCaseId}: ${result.overallScore.toFixed(3)} - ${testCase?.description || "Unknown"}`
          );
          console.log(
            `    Answer Correctness: ${result.answerCorrectness.toFixed(
              3
            )} | Exercise Design: ${result.exerciseDesign.toFixed(
              3
            )} | Explanations: ${result.explanationQuality.toFixed(3)}`
          );
        });
      }
    });
