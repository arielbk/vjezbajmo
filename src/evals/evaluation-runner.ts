...
export class EvaluationRunner {
  ...
  async runSingleTest(model: ModelConfig, testCase: GenerationTestCase): Promise<GenerationResult> {
    ...
    try {
      ...
      result.costEfficiency = this.scoreCostEfficiency(model, result.tokenUsage);
  ...
  private async generateExercise(model: ModelConfig, testCase: GenerationTestCase): Promise<Response> {
    ...
    const apiKey = model.provider === "openai" ? process.env.OPENAI_API_KEY : process.env.ANTHROPIC_API_KEY;
  ...
  private scoreCulturalRelevance(exercise: GeneratedExercise, _testCase: GenerationTestCase): number {
    ...
    const matches = culturalIndicators.filter((indicator) => exerciseText.includes(indicator)).length;
  ...
  async calculateModelPerformance(model: ModelConfig, results: GenerationResult[]): Promise<ModelPerformance> {
    ...
    if (successfulResults.length > 0) {
      criteria.answerCorrectness =
        successfulResults.reduce((sum, r) => sum + r.answerCorrectness, 0) / successfulResults.length;
      criteria.explanationQuality =
        successfulResults.reduce((sum, r) => sum + r.explanationQuality, 0) / successfulResults.length;
      criteria.exerciseDesign =
        successfulResults.reduce((sum, r) => sum + r.exerciseDesign, 0) / successfulResults.length;
      criteria.speedReliability =
        successfulResults.reduce((sum, r) => sum + r.speedReliability, 0) / successfulResults.length;
